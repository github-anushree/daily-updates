...
SELECT Category, MAX(sale_amount) AS max_amount
FROM sales
GROUP BY Category;
...
SELECT product_name, COUNT(sale_amount) as count_sales
from sales
group by product_name;

-- 6-	Which products have more than 5 sales?
SELECT product_name, COUNT(*) as count_sale
from sales
group by product_name
having count(*) > 5;

-- 7-	What is the average sale amount for products with more than 2 sales?
SELECT product_name, avg(sale_amount) AS avgSale,
COUNT(*) AS total_sale
FROM sales
group by product_name
having count(*) > 2;
...
select product_name , max(sale_amount) AS high_sale
from sales
group by product_name
...
SELECT month(sale_amount) as month_sale,
avg(sale_amount) as avg_sale_amount
from sales
group by month(sale_amount)
order by month_sale;

SELECT * FROM sales;
-- 12-	What are the products with the highest sale amount in each category?
SELECT product_name, category, sale_amount
from(
select product_name, category, sale_amount,
rank() over(partition by category order by sale_amount) as rnk
from sales
)
as ranked_sales
where rnk= 1;
...
select MONTH(sale_date) as month_sale,
...
select  category, avg(sale_amount) as avg_sale_amount,
month(sale_date) as month_sale,
count(*) as countsales
from sales
group by category, month(sale_date)
having countsales >3
order by category, month_sale;
