...
select * from cars
where price > (select avg(price) from cars); -- this is a subquery

-- we can do this manually, first we need to calculate the avg price of car
select avg(price) from cars; -- 57171.920000 (Answer)

select * from cars
where price > 57171.920000;

-- 1.1 List down the car which has max price
-- select * from cars
-- where price = max(price);  this is wrong, where ke sath aggregate func (max) nahi lagta thats why we need subquery
select max(price) from cars; -- find max price of cars in single query

select * from cars
where price = (select max(price) from cars); -- find max price of cars in sub query
...
select brand, count(model) as model_count 
from cars
group by brand
...
from cars
group by brand
having count(model) = (select brand, count(model) as model_count 
from cars
group by brand
order by model_count desc);
