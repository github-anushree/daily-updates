...
select o.orderid, o.productid, o.orderdate, o.quantity, o.totalamount, o.paymentstatus, c.firstname, c.lastname
from orders as o
join customers as c
...
-- if it is asked for unique product use DISTINCT
select distinct p.productname, p.category
from products as p;

-- 3. List the order details, including product names, quantities, and prices.
select o.orderid, o.ProductID, o.quantity, p.productname, p.price
from orders as o
join products as p
on o.productid = p.ProductID;

-- 4. Retrieve a list of customers who made orders and the order dates.
select c.firstname, c.lastname, o.orderid, o.productid, o.orderdate, o.quantity
from customers as c
join orders as o
...
select p.productname, p.Manufacturer, p.category
from products as p;
...
select c.firstname, c.lastname, c.country, o.orderid, p.productname, o.quantity
from customers as c
join orders as o
on c.customerid = o.customerid
join products as p
