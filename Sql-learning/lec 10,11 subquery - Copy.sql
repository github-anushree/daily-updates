...
select avg(price) from orders; -- '19981.944444'

-- manual method
-- select * from orders
-- where price > 19981.944444;
...
select paymentmethod, count(distinct(customername)) as customer_count
from orders
...
select customername , productname, city, price 
from orders o
where price > (select avg(price) 
from orders 
where city = o.city);

-- We use a correlated subquery — the inner query calculates the average price for that specific city (WHERE city = o.city).
-- The outer query then selects customers who have orders priced above that city’s average.
...
-- select count(productname) from orders
-- group by productname
-- having count(productname) >2; -- Your query is partually correct — it will show how many times each product appears, but it 
-- won’t return the full order details.
...
-- select price, city
-- from orders
-- where city = 'mumbai'; -- gives price with the city mumbai
...
select *
from orders 
where deliverydate < (select min(orderdate)
from orders 
